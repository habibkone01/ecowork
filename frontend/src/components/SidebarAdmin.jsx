        ...
        <>
            <div className="lg:hidden fixed top-0 left-0 right-0 z-50 px-4 pt-4">
                <div className="flex items-center justify-between px-5 py-3 rounded-full bg-[#1a1a2e] shadow-lg shadow-black/30">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold px-2 py-0.5 rounded-md bg-[#7bdff2] text-[#1a1a2e]">ADMIN</span>
                    </div>
                ...
                {open && (
                    <>
                        <div className="mt-3 rounded-2xl bg-[#1a1a2e] shadow-xl shadow-black/40 overflow-hidden border border-[#ffffff14]">
                            ...
                            <div className="border-t border-[#ffffff14] px-3 py-2">
                                <button onClick={() => { logout(); setOpen(false) }} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-[#dc2626] bg-transparent border-none cursor-pointer hover:bg-[#ef444411]"><LogOut size={16} />Se déconnecter</button>
                            </div>
                        </div>
            ...
            <aside className="hidden lg:flex fixed left-2 top-3 bottom-3 z-50 flex-col w-64 rounded-xl bg-[#1a1a2e] shadow-xl shadow-black/30 border border-[#ffffff14]">
                ...
                <nav className="flex-1 p-4 space-y-1">
                    <Link to="/admin/dashboard" className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all no-underline ${isActive('/admin/dashboard') ? 'bg-[#7bdff24d] text-white font-semibold' : 'text-[#ffffff8c]'}`}><LayoutDashboard size={16} />Dashboard</Link>
                    ...
                    <Link to="/admin/reservations" className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all no-underline ${isActive('/admin/reservations') ? 'bg-[#7bdff24d] text-white font-semibold' : 'text-[#ffffff8c]'}`}><Calendar size={16} />Réservations</Link>

                    <Link to="/admin/utilisateurs" className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all no-underline ${isActive('/admin/utilisateurs') ? 'bg-[#7bdff24d] text-white font-semibold' : 'text-[#ffffff8c]'}`}><Users size={16} />Utilisateurs</Link>
                </nav>
                ...
                <div className="p-4 border-t border-[#ffffff14]">
                    ...
                    <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all bg-transparent border-none cursor-pointer text-[#dc2626]"><LogOut size={16} />Se déconnecter</button>
                </div>
            </aside>
        </>
    )
